...
st.set_page_config(page_title="Bagging Regressor", layout="wide")

# "seaborn-bright" was renamed in modern matplotlib; fall back gracefully
# if the exact style string isn't available in the installed version.
try:
...
n_estimators = int(st.sidebar.number_input(
    'Enter number of estimators', min_value=1, max_value=500, value=10, step=1
))

# max_samples must be > 0, so the slider can no longer start at 0
max_samples = st.sidebar.slider('Max Samples', 25, 150, 150, step=25)
